...
- **ApplicationSets**: Generate Applications dynamically from templates, enabling patterns like "deploy to all namespaces matching this label" (see
  [`GitOps, Policy, and Eventing`](../../../concepts/gitops-model.md) for the App‑of‑AppSets model)
- **Sync Waves**: Orchestrate deployment order (namespaces before CRDs, CRDs before applications); see the Sync Waves section in
...
See the [Configuration Values](#values) section below for Redis resource tuning.
