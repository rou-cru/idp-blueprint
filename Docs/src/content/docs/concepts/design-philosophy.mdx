...
- **Sync Waves:** Dependencies are handled declaratively via
  ...
  [`GitOps, Policy, and Eventing`](gitops-model) for the wave model used
  here)
- **ApplicationSets:** Dynamic application generation from templates (see
  [`GitOps, Policy, and Eventing`](gitops-model) for the App‑of‑AppSets
  pattern)
...
- See [GitOps Model](gitops-model) for ApplicationSets, sync waves, and the App-of-AppSets pattern
- See [Application Architecture](../architecture/applications) for technical implementation details
