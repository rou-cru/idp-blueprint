...
A sensor subscribed to remediation events filters incoming alerts by label. It checks for `slo="argocd-application-health"` and extracts the application name and namespace from the alert payload. With this context, the sensor triggers a workflow that syncs the ArgoCD Application, forcing reconciliation. If the application was merely out of sync due to a transient Git operation, the sync resolves it. If the underlying issue persists, subsequent alerts provide escalation paths—notification to on-call engineers, automatic rollback, or diagnostic data collection.
...
See [Components - Argo Events](/components/eventing/argo-events/) for configuration details.
