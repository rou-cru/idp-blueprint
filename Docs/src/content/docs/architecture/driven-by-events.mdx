...
for `slo="argocd-application-health"` and extracts the `name` and `dest_namespace`
labels from the alert payload. With this context, the sensor triggers a workflow that syncs the
...
4. Sensor extracts `name` and `dest_namespace` from alert labels
5. Argo Workflow executes: `argocd app sync <name> --namespace <dest_namespace>`

See [Components - Argo Events](/implementation/components/eventing/argo-events/) for configuration
details.
