...
- [GitOps model](](../concepts/gitops-model)
- [Security & policy model](](../concepts/security-policy-model)
...
See: [GitOps model](](../concepts/gitops-model), [Secrets](secrets), [Policies](policies).

## 2. Gateway API service exposure

![Gateway API](../assets/diagrams/architecture/gateway-api.svg)

See: [Networking & gateway](](../concepts/networking-gateway).
...
See: [Getting Started](](../getting-started/quickstart) for bootstrap details.
...
See: [Secrets architecture](secrets) for detailed secret management.
...
See: [GitOps model](](../concepts/gitops-model) and [Policy model](](../concepts/security-policy-model).
