...
The eventing stack provides event-driven automation and orchestration capabilities
for the platform, enabling reactive workflows and intelligent system responses.

## Overview

Event-driven architecture decouples producers from consumers, letting the platform
react to changes in Kubernetes, Git repositories, external systems, and monitoring
...
- **[Argo Events](argo-events.mdx)**: Event-driven dependency manager. Automation
  for Kubernetes
...
- **Event Ingestion**: Multiple event source types (webhooks, Kubernetes resources,
  message queues, cloud events)
...
See the [Argo Events documentation](argo-events.mdx) for detailed configuration,
event source patterns, sensor recipes, and operational guidance.
