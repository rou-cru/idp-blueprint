...
See [CI/CD Model](../../architecture/cicd.md) for how Argo Events integrates into the
platform automation.
...
| Path | Purpose |
|------|---------|
| `K8s/events/governance/` | Namespace and resource limits applied early for capacity reservation. |
| `K8s/events/argo-events/values.yaml` | Helm values for controller + webhook with priority, tolerations, etc. |
...
- Both controller and webhook use the `platform-events` PriorityClass and pin to control-plane
  nodes through node affinity and tolerations so automation continues even if worker nodes are
  under pressure (`values.yaml`).
- Rolling updates use `maxUnavailable: 0` / `maxSurge: 1` to enforce zero-downtime restarts for
  the controller.
- The EventBus (`eventbus.yaml`) provisions a three-node NATS cluster with token auth; it is
  the single default bus referenced by all Sensors.
...
3. Label everything with the canonical metadata (`app.kubernetes.io/part-of: idp`,
   `owner: platform-team`, etc.) and include governance files if the folder introduces its own
   namespace.
4. Commit and let the ApplicationSet reconcile it—no manual registration is required.

Common recipes:

- **Git webhooks** → trigger Argo Workflows templates or invoke the ArgoCD API for intelligent
  syncs.
