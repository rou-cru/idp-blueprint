...
- **Secrets**: ExternalSecrets from Vault (`secret/backstage/app`, `secret/backstage/postgres`), using a dedicated `SecretStore` and SA `external-secrets`.
- **Exposure**: `HTTPRoute` `backstage.nip.io` on the platform gateway (TLS terminates at the Gateway).
- **Monitoring**: `ServiceMonitor` labeled `prometheus: kube-prometheus` scraping service `backstage` port `http`.
- **Operations**:
