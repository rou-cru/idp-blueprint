...
The platform front-loads tagging so you can attribute resource usage—even on a laptop
demo—before expanding to cloud cost tools.
...
1. **Validate locally** using `Scripts/validate-consistency.sh` – it checks for missing
   labels.
2. **Propagate to external tools** by mapping labels to e.g.,
   `kubecost.cloud.google.com/team` via relabeling if you export metrics.
