...
These defaults come from the demo profile (`config.toml`) and can be overridden per environment.
Keep the keys; adjust the values as needed for your org.
...
[`Scheduling, Priority, and Node Pools`](../concepts/scheduling-nodepools).
