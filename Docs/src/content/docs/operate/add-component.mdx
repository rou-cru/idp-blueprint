...
3. Apply canonical labels (`owner`, `business-unit`, `environment`,
   `app.kubernetes.io/*`).
4. Commit + push.
...
1. `namespace.yaml` — canonical labels (`app.kubernetes.io/part-of`, `owner`,
   `business-unit`, `environment`) and sync wave annotation:
...
This overlay is part of the platform contract ("Namespace governance" in
[Contracts & Guardrails](contracts)); omitting it will fail reviews and breaks capacity
planning.
