...
description: Feature Toggles (Fuses)
...
- `task stacks:deploy` reads fuses and applies only the enabled stacks (events are
  always on).
- `fuses.prod = true` enables production hardening (today: HA for ArgoCD; future: more
  dials).
...
- Observability: `alertmanager.enabled`, `loki.enabled`, `fluent-bit.enabled`,
  `pyrra.enabled`
...
- Contracts & Guardrails: [operate/contracts.md](contracts)
