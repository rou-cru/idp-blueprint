...
ArgoCD admin password is **stored as a bcrypt hash** in the `argocd-secret` created by External Secrets → Vault. Reading it directly will return the hash, which is not usable to log in.

For the demo the usable password lives in `config.toml` under `passwords.argocd_admin` (default: `argo`). If you change it there and redeploy, ArgoCD will be seeded with the new value.

- Username: `admin`
- Password: value of `passwords.argocd_admin` in `config.toml`

If you need to verify what ArgoCD is using, check `config.toml`; do **not** rely on `kubectl get secret ... admin.password` because it will show the bcrypt hash.

#### CLI login (desde tu LAN)

El servidor ArgoCD está en modo `insecure` (HTTP detrás del Gateway), así que el CLI necesita `--plaintext` contra el hostname público:

```bash
argocd login argocd.<tu-ip-dashed>.nip.io --username admin --password "$(dasel -f config.toml -r toml -w json .passwords.argocd_admin)" --plaintext
```

Reemplaza `<tu-ip-dashed>` por el valor que imprimió `task deploy` (ej. `192-168-65-16`).
