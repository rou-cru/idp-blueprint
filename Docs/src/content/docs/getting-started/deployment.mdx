...
- Apply policies and let ApplicationSets sync the stacks – ApplicationSets
  ...
  `K8s/` using sync waves to control deployment order (see [GitOps Model](../concepts/gitops-model.md#sync-waves--ordering-without-scripts) for wave definitions). Stack deployment is controlled by [fuses](../overview/#feature-toggles-fuses) defined in `config.toml`.

### Platform Components (mirrors the Taskfile flow)
