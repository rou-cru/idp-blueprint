...
Now that the deploy command has run, let's confirm the platform is coming up and set
realistic expectations for the first few minutes.

## What to expect in the first 5–10 minutes

ArgoCD keeps syncing after `task deploy` exits, pods progress from Pending to Ready as
images download, and it is normal for Health/Sync to settle gradually rather than
instantly.
