...
<div class="flex justify-center gap-3 mt-8">
  ...
  {/* No Next button here, as this is the start of a new section */}
</div>
...
These philosophies reinforce each other. GitOps builds on Infrastructure as Code.
Security as Code leverages declarative configuration. Observability as Code makes the
entire system's behavior visible.

## 1. Declarative Configuration

### What It Means — Declarative Configuration

Instead of writing scripts that execute a sequence of steps (imperative), you declare
what the desired end state should look like. The system figures out how to achieve that
state.
...
### How It Manifests — Declarative Configuration
...
- **Kyverno Policies:** Policies are ClusterPolicy CRDs, not imperative validation
  scripts
- **ServiceMonitors:** Prometheus scrape configuration is declarative CRDs, not config
  files

**Benefits:**

- **Idempotent:** Applying the same manifest multiple times produces the same result
- **Self-Healing:** If the current state drifts from desired state, the system
  automatically corrects it
- **Predictable:** You can reason about what the system will do by reading the manifests

## 2. Infrastructure as Code

### What It Means — Infrastructure as Code

Infrastructure (networks, compute, storage, services) is defined in code files rather
than configured through UIs or manual processes.

### How It Manifests — Infrastructure as Code
...
- **Code Review:** Infrastructure changes go through the same review process as
  application code
...
- **Sync Waves:** Dependencies are handled declaratively via
  ...
  [`GitOps, Policy, and Eventing`](gitops-model) for the wave model used here)
- **ApplicationSets:** Dynamic application generation from templates (see
  [`GitOps, Policy, and Eventing`](gitops-model) for the App‑of‑AppSets pattern)
...
- **Rollback:** `git revert` to undo changes Disaster recovery becomes a matter of
  re-deploying from Git if the cluster is lost. This model fosters collaboration by
  allowing multiple engineers to propose changes via pull requests, ensuring consistency
  by eliminating environment-specific manual steps.

**Next Steps:**

- See [GitOps Model](gitops-model) for ApplicationSets, sync waves, and the
  App-of-AppSets pattern
- See [Application Architecture](../architecture/applications) for technical
  implementation details

## 4. Security as Code

### What It Means — Security as Code
...
### How It Manifests — Security as Code
...
- **Secrets Management:** Secrets flow through Vault and External Secrets, never
  hardcoded
...
### What It Means — Observability as Code

Monitoring, logging, and alerting configurations are declarative and version-controlled,
not configured through UIs.

### How It Manifests — Observability as Code

- **ServiceMonitors:** Prometheus scrape targets are defined as CRDs. Deploy a service
  with a ServiceMonitor, Prometheus discovers it automatically.
...
A new service wants to expose metrics. Instead of manually updating Prometheus config
files:
...
![Design Philosophy Flow](../assets/diagrams/implementation/design-philosophy-flow.svg)

> **Source:**
> [design-philosophy-flow.d2](../assets/diagrams/implementation/design-philosophy-flow.d2)
...
- **Do:** Store secrets in Vault, define ExternalSecrets, reference the synced
  Kubernetes Secret
...
- **Learning Curve:** Understanding declarative patterns and GitOps requires initial
  investment
- **Indirection:** Debugging can be harder when automation is doing things behind the
  scenes
...
- [GitOps Model](gitops-model): Deep dive into the GitOps workflow
- [Security & Policy Model](security-policy-model): How Security as Code manifests
- [Observability Model](../architecture/observability): How Observability as Code works
